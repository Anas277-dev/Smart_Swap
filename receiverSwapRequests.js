...
router.post("/accept", async (req, res) => {
  try {
    ...
    const receiverProfile = await pool.query(
      "SELECT full_name FROM user_profile WHERE profile_id=$1",
      [receiver_profile_id]
    );
    const receiverName = receiverProfile.rowCount ? receiverProfile.rows[0].full_name : "Someone";

    await pool.query(
      `INSERT INTO notifications (user_id, title, message, type)
       VALUES ($1, $2, $3, 'swap')`,
      [
        updatedRequest.sender_user_id,
        "Swap Request Accepted",
        `${receiverName} accepted your swap request.`
      ]
    );
  ...
  try {
    ...
    await pool.query(
      ...
      [
        ...
        `${receiverName} rejected your swap request.`
      ]
    );
