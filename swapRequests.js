...
router.post("/", async (req, res) => {
  try {
    ...
    // 🔹 Insert swap requests and notifications
    const insertedRequests = [];
    for (let i = 0; i < sender_subject_ids.length; i++) {
      const result = await pool.query(
        `INSERT INTO swap_requests 
      ...
      const senderName = senderProfile.rowCount ? senderProfile.rows[0].full_name : "Someone";
      ...
      await pool.query(
        ...
        [
          ...
          `${senderName} sent you a swap request.`
        ]
      );
    }
